...
if [[ -d "$SWIFT_DIR" ]]; then
  ...
  # Detect host architecture and map to Rust triple
  HOST_ARCH=$(uname -m)
  ...
  echo "[sidecar] Building Swift Parakeet sidecar ($SWIFT_ARCH release)..."
  pushd "$SWIFT_DIR" > /dev/null
  swift build -c release --arch "$SWIFT_ARCH"
  BIN_DIR=$(swift build -c release --arch "$SWIFT_ARCH" --show-bin-path 2>/dev/null || echo ".build/${SWIFT_ARCH}-apple-macosx/release")
  SRC_BIN_NAME="ParakeetSidecar"
  SRC_BIN_PATH="$BIN_DIR/$SRC_BIN_NAME"
  mkdir -p dist
  cp "$SRC_BIN_PATH" "dist/parakeet-sidecar-$RUST_TRIPLE"
  chmod +x "dist/parakeet-sidecar-$RUST_TRIPLE"
  ln -sfn "parakeet-sidecar-$RUST_TRIPLE" "dist/parakeet-sidecar"
