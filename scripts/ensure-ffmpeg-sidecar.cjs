...
(function main() {
  ...
  if (isMac) {
    ...
    const archTriple = isArm ? 'aarch64-apple-darwin' : 'x86_64-apple-darwin';
    console.log(`[ensure-ffmpeg-sidecar] macOS detected, arch=${process.arch}, triple=${archTriple}`);

    const ffmpegDst = path.join(distDir, 'ffmpeg');
    const ffprobeDst = path.join(distDir, 'ffprobe');
    const ffmpegArchLink = path.join(distDir, `ffmpeg-${archTriple}`);
    const ffprobeArchLink = path.join(distDir, `ffprobe-${archTriple}`);

    if (fs.existsSync(ffmpegDst) && fs.existsSync(ffprobeDst)) {
      // Ensure arch-specific symlinks even if binaries already present
      ensureSymlink(ffmpegDst, ffmpegArchLink);
      ensureSymlink(ffprobeDst, ffprobeArchLink);
      console.log('[ensure-ffmpeg-sidecar] ffmpeg/ffprobe sidecars present. Symlinks ensured.');
      return;
    }

    // Pinned macOS binaries per architecture (override via env if needed)
    const ffmpegZipUrl = process.env.FFMPEG_MAC_URL || (isArm
      ? 'https://www.osxexperts.net/ffmpeg80arm.zip'
      : 'https://www.osxexperts.net/ffmpeg80intel.zip');
    const ffprobeZipUrl = process.env.FFPROBE_MAC_URL || (isArm
    ...
    // Use arch-specific env vars, falling back to the matching pinned hash
    const macFfmpegBinSha = process.env.FFMPEG_MAC_BIN_SHA256 || (isArm
      ? '77d2c853f431318d55ec02676d9b2f185ebfdddb9f7677a251fbe453affe025a'
      : null);
    const macFfprobeBinSha = process.env.FFPROBE_MAC_BIN_SHA256 || (isArm
    ...
    ensureSymlink(ffmpegDst, ffmpegArchLink);
    ensureSymlink(ffprobeDst, ffprobeArchLink);
    console.log(`[ensure-ffmpeg-sidecar] Installed macOS (${archTriple}) sidecar binaries by download.`);
    return;
  }
